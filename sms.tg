...
cek_kuota() {
    ...
    if [ -n "$HASIL" ] && [ "$HASIL" != "null" ]; then
        ...
        FILTERED=$(echo "$CLEANED" | awk '/🎁 Quota:/,/🌲 Sisa Kuota/')

        if [ -n "$FILTERED" ]; then
            MESSAGE=$(printf "📊 Info Kuota untuk %s:\n\n%s" "$MSISDN" "$FILTERED")
            send_telegram "$MESSAGE"
        else
            send_telegram "❌ Tidak ditemukan detail kuota."
        fi
    else
    ...
    while true; do
        ...
        if [ "$DATE" != "$LAST_DATE" ] && [ -n "$BODY" ]; then
            ...
            if echo "$BODY" | grep -q "$TRIGGER_ACTIVE"; then
                ...
                cek_kuota   # langsung cek kuota setelah paket aktif
            fi
        fi

        # --- Cek kuota tiap 3 jam ---
        NOW=$(date +%s)
        if [ $((NOW - LAST_QUOTA_CHECK)) -ge 10800 ]; then
            cek_kuota
